...
def create_user():
    try:
        ...
        dbResponse = db.students.insert_one(user)
...
def update_user(id):
    try:
        dbResponse = db.students.update_one({"_id": ObjectId(id)}, {"$set": {"name": request.form["name"]}})
        for attr in dir(dbResponse):
            print(f"********{attr}********")
        return Response(
...
def delete_user(id):
    try:
        dbResponse = db.students.delete_one({"_id": ObjectId(id)})
        for attr in dir(dbResponse):
            print(f"********{attr}********")
        return Response(
